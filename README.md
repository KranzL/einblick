...
Install the plugin and run `/sqlscout`. Claude reads the data and writes the report inside your conversation. **No LLM API key from you** -- the agent you're already talking to does that work.

```
/plugin marketplace add KranzL/sqlscout
/plugin install sqlscout@sqlscout
/sqlscout --platform snowflake --days 7
```
...
```bash
git clone https://github.com/KranzL/sqlscout
bash sqlscout/hooks/scripts/install.sh
...
sqlscout/.venv/bin/sqlscout analyze --sample \
  --provider anthropic --output sample-report.md

# Real run:
export SQLSCOUT_ANTHROPIC_API_KEY=sk-ant-...
export SNOWFLAKE_ACCOUNT=...
export SNOWFLAKE_USER=...
export SNOWFLAKE_PASSWORD=...

sqlscout/.venv/bin/sqlscout analyze \
  --platform snowflake \
  --days 7 \
  --output report.md
```

> Use `SQLSCOUT_ANTHROPIC_API_KEY`, **not** the bare `ANTHROPIC_API_KEY`. The unprefixed one is read by Claude Code itself, so putting it in your shell profile will hijack Claude Code and break its auth. The `SQLSCOUT_`-prefixed name is sqlscout-only. The bare name still works as a fallback for one-off invocations (Airflow tasks, etc.) but you'll get a warning telling you to switch.

For non-Anthropic providers (Venice.ai, Together, local Ollama), point `--llm-base-url` at the compatible endpoint:

```bash
export SQLSCOUT_OPENAI_API_KEY=your-venice-key

sqlscout analyze --provider openai \
...
BashOperator(
    ...
    bash_command="sqlscout analyze --platform snowflake --days 7 --output /tmp/report.md",
    env={
        "SQLSCOUT_ANTHROPIC_API_KEY": "{{ var.value.anthropic_key }}",
...
For scheduled runs, the cleanest deployment is the prebuilt container at `ghcr.io/kranzl/sqlscout:latest`. Pinned Python, pinned deps, no virtualenv to babysit on shared infrastructure.

```bash
docker run --rm \
  -e SQLSCOUT_ANTHROPIC_API_KEY=sk-ant-... \
  -e SNOWFLAKE_ACCOUNT=... \
  -e SNOWFLAKE_USER=... \
  -e SNOWFLAKE_PASSWORD=... \
  -e SQLSCOUT_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/... \
  -v $(pwd)/reports:/workspace/reports \
  ghcr.io/kranzl/sqlscout:latest \
...
KubernetesPodOperator(
    task_id="sqlscout_weekly",
    image="ghcr.io/kranzl/sqlscout:latest",
    arguments=[
        "analyze", "--platform", "snowflake", "--days", "7",
        "--slack-mode", "alert",
        "--output", "/workspace/reports/sqlscout.md",
    ],
    env_vars={
        "SQLSCOUT_ANTHROPIC_API_KEY": Variable.get("anthropic_key"),
        "SNOWFLAKE_ACCOUNT": Variable.get("snowflake_account"),
        "SNOWFLAKE_USER": Variable.get("snowflake_user"),
        "SNOWFLAKE_PASSWORD": Variable.get("snowflake_password"),
        "SQLSCOUT_SLACK_WEBHOOK_URL": Variable.get("sqlscout_slack_webhook"),
    },
    volume_mounts=[VolumeMount("reports", "/workspace/reports", read_only=False)],
)
```

The image runs as a non-root user (`sqlscout`), has all three platforms baked in, and ships at ~250MB. No tag = `latest`; pin to a version (`ghcr.io/kranzl/sqlscout:0.2.0`) for reproducible cron runs.

The interactive Claude Code path stays as-is -- the slash command runs the local venv directly, no container involved.

## Slack delivery

Add `--slack-webhook` (or `SQLSCOUT_SLACK_WEBHOOK_URL` env var) and sqlscout posts a Block Kit summary to Slack after the report is written. **No Slack app needed -- you create the webhook yourself in your own workspace.** It's a 90-second setup:
...
4. Set it as `SQLSCOUT_SLACK_WEBHOOK_URL` and you're done

```bash
export SQLSCOUT_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T0X/B0Y/abc123"

sqlscout analyze --platform snowflake --days 7 --slack-mode alert --output /tmp/r.md
```
...
Set three env vars, pass `--dbt-aware`. sqlscout calls dbt Cloud's Discovery API directly to grab the model list, lineage, and per-model perf numbers. If auth fails or dbt Cloud is down, the run still finishes -- you just don't get the dbt context.
...
sqlscout analyze --platform snowflake --days 7 --dbt-aware --output report.md
```
...
This works for both `/sqlscout` (interactive) and `sqlscout analyze` (cron). The slash command will ask if you want to use dbt context; the CLI uses it whenever you pass `--dbt-aware`.

### Path 2: full integration via dbt-mcp (Claude Code only)

Beyond just reading dbt's data, you can let the agent actually create the proposed models -- write the `.sql` and `schema.yml`, run `dbt parse / compile / run / test`, and open a PR. This requires dbt's official MCP server alongside sqlscout's plugin.
...
- `DBT_DEV_ENV_ID` -- the env sqlscout will materialize against during validation. **Use a dev environment, not prod.** Same place as the prod env ID, just click the dev one.
...
You also need the `dbt-codegen` package in your project's `packages.yml`. sqlscout uses it to generate `schema.yml` for new models:
...
**What happens when it's all wired up.** When you run `/sqlscout` and pick "yes" on the dbt context question, the slash command does the analysis as usual. After the report is written, it offers to actually apply the proposed changes:

1. Asks which proposed models you want to create
2. Asks which dbt target to test against (asks once, persists to `~/.sqlscout.yml`)
...
If you want sqlscout to open dbt PRs at 3am unattended, set up a **dedicated GitHub service account** so the PRs are clearly automation:

1. Create a GitHub user like `sqlscout-bot`
...
Don't reuse your personal GitHub account. PRs from a real human imply that human reviewed the change. PRs from `sqlscout-bot` signal "automation, review the diff not the author."

## What you need to run sqlscout at all
...
The dbt parts above are optional -- sqlscout works fine without them.

### MotherDuck specifics

```bash
export MOTHERDUCK_TOKEN=your_admin_token   # Org Settings -> Create token
sqlscout setup --platform motherduck
sqlscout extract --platform motherduck --hours 24 --output /tmp/md.md --format markdown
```
...
Don't want to type the same flags every time? Drop a `~/.sqlscout.yml` (your defaults) or `.sqlscout.yml` in a repo (project defaults). CLI flags win over both.
...
```bash
sqlscout extract --sample --format markdown --output /tmp/out.md
```

Ships with 775 realistic Snowflake queries so you can see the output shape before pointing it at prod. `sqlscout analyze --sample` runs the same data through the full LLM pipeline -- handy for verifying your API key works and your model handles tool-calling correctly.

## Subcommands

```bash
sqlscout extract  --platform snowflake --days 7 --output results.json
sqlscout analyze  --platform snowflake --days 7 --output report.md
sqlscout setup    --platform snowflake                # test your connection
sqlscout users    --platform snowflake --hours 24     # list users, flag service accounts
sqlscout history  list --platform snowflake           # past runs
```

`sqlscout --help` or `sqlscout <subcommand> --help` for the full flag list. Common ones below.
...
| `--no-history` | Skip saving this run to `~/.sqlscout/history/` |
...
Every run drops the JSON result and markdown report into `~/.sqlscout/history/<platform>/<timestamp>.{json,md}`. Default retention is the last 12 runs per platform; tune with `--keep-history N` (or `0` for unlimited).

`sqlscout history list --platform snowflake` shows past runs newest-first. This sets up run-over-run diff later: "compared to last week, three new patterns appeared, pattern X got 40% cheaper."

For Airflow on ephemeral containers, point `SQLSCOUT_HISTORY_DIR=/mnt/shared-volume` so history survives between runs. Opt out per-run with `--no-history`.
...
- Two entry points: the `/sqlscout` slash command for interactive Claude Code / Codex sessions (no API key needed) and the `sqlscout analyze` CLI for cron / Airflow / GitHub Actions (Anthropic, OpenAI, or any OpenAI-compatible endpoint via `--llm-base-url`).
...
- Docker image (`ghcr.io/kranzl/sqlscout:latest`) for scheduled runs. Mount `./reports` and `./history` as volumes for persistence.
- History is kept under `~/.sqlscout/history/` (atomic writes, mode `0o600`, `0o700` directory) so run-over-run diffs work without you setting anything up.
