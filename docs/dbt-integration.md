...
How einblick hands off its recommendations to a user's dbt project.

## What this is for

Right now einblick produces a markdown report with recommendations. Users read it, decide what to do, write dbt models by hand, open a PR. That last mile is manual and is where most of the value leaks out -- the recommendation is only as good as the follow-through.

This document is the plan for closing that loop by integrating with the dbt MCP server, so einblick's output can become actual dbt model files, validated and opened as a PR, without leaving Claude Code.
...
- Platform users are also the group paying for dbt Cloud, which correlates with the kind of workload einblick is most useful on (bigger warehouses, more model volume).

Core users still get einblick's full report. The `## Proposed dbt Changes` section lands as copy-paste-ready SQL and YAML blocks they can drop into their project manually. They just don't get one-button creation.

## The three recommendation types

`einblick analyze` emits a structured `## Proposed dbt Changes` section. Every proposal fits one of three shapes:
...
Access patterns surface in the report but never auto-apply changes -- governance is political and not something einblick should touch unilaterally.
...
Every JSON block includes `"einblick_schema_version": "1"` so downstream parsers can detect and handle drift.
...
Result: the `## Proposed dbt Changes` section is always valid because einblick wrote it from typed data, not the LLM's prose. No try/catch around malformed JSON anywhere downstream.

**Scope:**
- Define the tool schema for each of the three recommendation types in `scripts/src/einblick/reporter.py`
- Update `skills/einblick-analysis/references/analysis-prompt.md` to instruct the LLM to call `emit_dbt_proposals` (tells it the tool exists, describes the fields, and says "always call this tool exactly once with your recommendations")
...
This is where the dbt integration earns its keep. Before the LLM writes recommendations, cross-reference einblick's patterns against the user's actual dbt models via the Discovery API.
...
4. For each einblick pattern, look up its source_tables; pick the most
...
5. For matched models (capped at 50 -- MAX_PERF_FETCHES), call
   ...
   per-model call.
6. Write to a uid-scoped temp file (e.g. /tmp/einblick-dbt-context-<uid>.json)
...
- **CLI:** `einblick analyze --dbt-aware` (new flag)
...
- New module `scripts/src/einblick/dbt_discovery.py` -- small GraphQL client for dbt's Discovery API (`metadata.cloud.getdbt.com/graphql`). ~100 lines, uses `requests`. Both the CLI and interactive paths call through this module -- single implementation, one auth path, one place to fix bugs. dbt-mcp is only strictly needed for Phase 2's workflow tools (parse/compile/run/test).
- Writes `/tmp/einblick-dbt-context.json` as the handoff file for Phase 2B

**Handoff file schema (`/tmp/einblick-dbt-context-<uid>.json`):**
...
3. Decide: does this change match a template?
   ...
   If a template matches:
     Use string-substitution helpers from scripts/src/einblick/dbt_edit_templates.py
...
9. dbt::get_model_performance (post-run capture)
   -> read "before" from /tmp/einblick-dbt-context.json (written in Phase 1)
...
1. Check `~/.einblick.yml` for `dbt_target:` -- persisted from a prior run
2. Else: on first invocation, call `dbt::list --output json` (or read `profiles.yml` directly) to enumerate available targets for the active profile. Show them via AskUserQuestion: "Which dbt target should einblick use for dev runs?"
3. Persist the choice to `~/.einblick.yml` so subsequent runs skip the question
...
**Branch naming.** `einblick/<slug>-<YYYYMMDD>` where `<slug>` is a short kebab-case summary (e.g., `revenue-incremental`). If two runs land on the same day for the same slug, append `-<HHMMSS>`.
...
- Fingerprints addressed: <list of einblick pattern IDs>
...
- Full report: <path to einblick_results_latest.md>
```
...
**Push failure fallback.** If `git push origin <branch>` fails (branch protection, no remote write, network), einblick:
...
Graceful degradation. A misconfigured dbt-mcp never blocks the core einblick value.
...
`einblick create-models --from-report report.md --dbt-project-dir ~/mydbt`

For Airflow / CI to act on einblick recommendations without a human at the keyboard. This is a larger piece of work than it looks -- it needs to re-implement Phase 2A/2B's orchestration (file writes, dbt parse/compile/run/test, git, gh) in Python rather than via Claude-in-the-session. Specifically:

- Read the structured proposals from the report file
- Call einblick's own Discovery client + dbt CLI subprocess for validation
...
- **Resolved dev target is always used.** Every `dbt run` through the flow hits the target einblick has resolved and persisted in `~/.einblick.yml` (see "Target resolution" in Phase 2 for how it's picked). Production materializations go through the user's normal CI, not einblick.
...
einblick itself doesn't have a GitHub identity. Whatever account `git` and `gh` are authenticated as on the machine running Claude Code is who shows up on the PR. Claude is just orchestrating `git commit` and `gh pr create` -- it's not an author. No `Co-Authored-By: Claude` trailers will be added.
...
- **Branch name**: `einblick/<short-description>` (e.g., `einblick/revenue-incremental-plus-stg-customer-events`). Stable prefix makes filtering in the GitHub UI / CODEOWNERS rules easy.
...
If you want einblick running on a schedule and opening PRs without a human at the keyboard, **you need a dedicated GitHub service user**. Don't reuse a personal account -- PRs authored by you while you're asleep will be confusing, and if you ever leave the team, the automation dies with your account.

What "dedicated service user" means:

1. **Create a GitHub user account** just for automation. Conventions: `einblick-bot`, `<your-org>-bot`, or whatever your org uses.
...
4. **Configure git identity** on the runner so commits are attributed to the bot:
   ```bash
   git config user.name "einblick-bot"
   git config user.email "einblick-bot@your-org.com"
   ```
...
6. **Set the PR opener identity** via `GH_TOKEN`:
   ...
   BashOperator(
       ...
       bash_command="einblick analyze ... && git push && gh pr create ...",
       env={
           "GH_TOKEN": "{{ var.value.einblick_bot_github_token }}",
           "GIT_AUTHOR_NAME": "einblick-bot",
           "GIT_AUTHOR_EMAIL": "einblick-bot@your-org.com",
           "GIT_COMMITTER_NAME": "einblick-bot",
           "GIT_COMMITTER_EMAIL": "einblick-bot@your-org.com",
           "DBT_USER_ID": "{{ var.value.einblick_bot_dbt_user_id }}",
           "DBT_PROJECT_DIR": "{{ task_instance.xcom_pull(task_ids='checkout_dbt_repo') }}",
           # ... plus EINBLICK_ANTHROPIC_API_KEY, SNOWFLAKE_*, DBT_TOKEN, DBT_HOST, DBT_PROD_ENV_ID
       },
   )
   ```
...
7. **Tell the team** the bot exists. PRs from `einblick-bot` should be treated as automation -- reviewed and merged like any other, but nobody should expect the bot to respond to review comments or iterate.
...
Claude didn't write the code -- einblick did, and the LLM was a means to that end. Adding Claude as a co-author on every commit creates noise, confuses git blame, and doesn't reflect who actually owns the change. The interactive user (or the bot account, for automation) is the author. Full stop.
...
| Structured output mechanism | Anthropic `tool_use` -- LLM calls `emit_dbt_proposals`, einblick renders the markdown from the typed data |
| Discovery API client location | Single GraphQL client in `scripts/src/einblick/dbt_discovery.py`, used by both CLI and interactive paths |
| schema.yml strategy for new models | Per-model `<name>.yml` file -- never edit existing schema.yml |
| Branch naming | `einblick/<slug>-<YYYYMMDD>` with `HHMMSS` fallback for same-day dupes |
| Misconfigured dbt-mcp fallback | Fall back to non-dbt-aware run + warning; never hard-fail the whole run |
| dbt target resolution | Ask on first run, persist to `~/.einblick.yml`; never assume `dev` |
...
- Where to store a "proposal -> PR" mapping so run-over-run diff can mark recommendations as implemented. Likely a line appended to `~/.einblick/history/<platform>/<timestamp>.applied.json` after a successful PR. Defer to the run-over-run diff PR.
- Whether `--dbt-aware` should persist in `~/.einblick.yml` so users don't have to pass it every run. Lean yes; implement when we wire the flag.
