...
class TestConnectSnowflake:
    @patch("einblick.connector.load_snowflake_credentials")
    ...
    def test_connects_with_password(self, mock_sf_connect, mock_creds):
        ...
        config = EinblickConfig(platform="snowflake")
    ...
    @patch("einblick.connector.load_snowflake_credentials")
    @patch("snowflake.connector.connect")
    @patch("einblick.connector._is_non_interactive", return_value=False)
    ...
    @patch("einblick.connector.load_snowflake_credentials")
    @patch("snowflake.connector.connect")
    @patch("einblick.connector._is_non_interactive", return_value=True)
