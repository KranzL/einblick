...
description: >
  ...
  reduce query costs, or mentions einblick.
version: 0.1.0
---
...
```bash
${CLAUDE_PLUGIN_ROOT}/.venv/bin/einblick extract --platform <snowflake|databricks|motherduck> --days <N> --format json --output "/tmp/einblick-results-$(id -u).json"
```

Then read `/tmp/einblick-results-$(id -u).json` (resolve `$(id -u)` to a numeric value via `Bash` first, then pass the literal path to `Read`). The uid suffix avoids cross-user collisions on shared boxes.
...
Every report ends with a `## Proposed dbt Changes` section. It's not free-form prose -- the LLM calls a typed tool named `emit_dbt_proposals` which einblick validates and renders. Three recommendation shapes:
...
- `access_pattern` -- users querying raw tables that a curated mart already covers (surface only; einblick never auto-applies governance changes)

Each proposal carries `metrics_addressed` (einblick pattern fingerprints it absorbs) and `einblick_schema_version` for forward compatibility.

Claude Code users with `dbt-mcp` installed can act on these proposals directly: create the model files, validate with `dbt parse` / `compile` / `run` / `test`, and open a PR -- all without leaving the conversation. See `docs/dbt-integration.md` in the einblick repo for the full flow. This lands phase-by-phase.
